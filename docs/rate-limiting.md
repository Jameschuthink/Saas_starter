...
#### 1. Rate Limit Module (`lib/saas_template_web/rate_limit.ex`)
```elixir
defmodule SaasTemplate.RateLimit do
...
#### 2. Application Supervision (`lib/saas_template/application.ex`)
...
#### 3. Endpoint Integration (`lib/saas_template_web/endpoint.ex`)
...
defp authenticated_rate_limit(conn, _opts) do
  ...
  case SaasTemplateWeb.RateLimit.hit(key, :timer.minutes(1), limit) do
    {:allow, _count} -> conn
    {:deny, retry_after} ->
      conn
      |> put_resp_header("retry-after", Integer.to_string(div(retry_after, 1000)))
...
```elixir
defmodule SaasTemplateWeb.RateLimitTest do
...
SaasTemplateWeb.RateLimit.inspect_bucket("web_requests:192.168.1.1", :timer.minutes(1))

# Manual rate limit testing in IEx
iex> SaasTemplateWeb.RateLimit.hit("test_key", :timer.minutes(1), 10)
{:allow, 1}
