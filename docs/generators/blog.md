...
- **SaasTemplate.Blog** (`lib/saas_template/blog.ex`) - Context module for blog functionality
- **SaasTemplate.Blog.Post** (`lib/saas_template/blog/post.ex`) - Post schema with auto-generation features
- **BlogController** (`lib/saas_template_web/controllers/blog_controller.ex`) - Public blog routes
- **BlogHTML** (`lib/saas_template_web/controllers/blog_html.ex`) - HTML rendering module

### Admin Interface
- **Admin Post LiveView** (`lib/saas_template_web/live/admin/post_live.ex`) - Backpex-powered blog management
...
### Application Updates
- **Router** (`lib/saas_template_web/router.ex`) - Public blog routes and admin Backpex integration
- **Layouts** (`lib/saas_template_web/components/layouts.ex`) - Blog admin layout function
- **Core Components** (`lib/saas_template_web/components/core_components.ex`) - Backpex translation functions
- **Root HTML** (`lib/saas_template_web/components/layouts/root.html.heex`) - SEO meta tag support
...
SaasTemplate.Blog.list_published_posts()

# Get a published post by slug
SaasTemplate.Blog.get_post_by_slug("my-blog-post")
```
...
SaasTemplate.Blog.Post.render_content(post)

# Render specific field (content or excerpt)
SaasTemplate.Blog.Post.render_content(post, :excerpt)
```
...
{:ok, post} = SaasTemplate.Blog.Post.changeset(%SaasTemplate.Blog.Post{}, %{
...
belongs_to :author, SaasTemplate.Accounts.User, foreign_key: :author_id
