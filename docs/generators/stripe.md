...
#### Purchase Schema (`lib/saas_template/purchases/purchase.ex`)
```elixir
...
#### Purchases Context (`lib/saas_template/purchases.ex`)
...
#### Stripe Webhook Controller (`lib/saas_template_web/controllers/stripe_webhook_controller.ex`)
...
SaasTemplate.Purchases.get_purchase!(123)
...
{:ok, purchase} = SaasTemplate.Purchases.create_or_update_purchase_from_payment_intent(payment_intent)
```
...
user = SaasTemplate.Accounts.get_user!(1)
purchases = SaasTemplate.Purchases.list_purchases()
|> Enum.filter(&(&1.user_id == user.id))

# Get purchase metadata
purchase = SaasTemplate.Purchases.get_purchase!(123)
