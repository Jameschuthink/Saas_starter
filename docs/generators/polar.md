...
- **Purchase schema** (`lib/saas_template/purchases/purchase.ex`) - Comprehensive schema for storing Polar.sh transaction data
- **Migration file** (`priv/repo/migrations/*_add_polar_purchases.exs`) - Database migration with proper indexes
- **Purchases context** (`lib/saas_template/purchases.ex`) - Context module for purchase management

### Web Layer Components
- **PolarWebhookController** (`lib/saas_template_web/controllers/polar_webhook_controller.ex`) - Handles incoming Polar.sh webhooks
- **Router updates** (`lib/saas_template_web/router.ex`) - Adds webhook endpoint at `/api/webhooks/polar`

### Test Support
- **Purchase tests** (`test/saas_template/purchases_test.exs`) - Comprehensive test coverage for purchase context
- **Webhook tests** (`test/saas_template_web/polar_webhook_controller_test.exs`) - Test coverage for webhook handling
...
The `SaasTemplate.Purchases` context provides the following functions:
...
# Get purchase by ID
SaasTemplate.Purchases.get_purchase!(id)
...
SaasTemplate.Purchases.create_purchase(%{
...
SaasTemplate.Purchases.update_purchase(purchase, %{status: "succeeded"})

# Create or update from Polar order data
SaasTemplate.Purchases.create_or_update_purchase_from_order(order_data)

# Create or update from Polar checkout data
SaasTemplate.Purchases.create_or_update_purchase_from_checkout(checkout_data)
```
