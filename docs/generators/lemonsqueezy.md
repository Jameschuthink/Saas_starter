...
```bash
mix saas_template.gen.lemonsqueezy --yes
```
...
- **Purchase schema** (`lib/saas_template/purchases/purchase.ex`) - Comprehensive schema for LemonSqueezy order data
...
- **SaasTemplate.Purchases** (`lib/saas_template/purchases.ex`) - Context module for purchase management
- **LemonSqueezyWebhookHandler** (`lib/saas_template_web/controllers/lemon_squeezy_webhooks_controller.ex`) - Webhook event handler

### Application Updates
- **Endpoint** (`lib/saas_template_web/endpoint.ex`) - Webhook plug added at `/webhook/lemonsqueezy`
- **Factory** (`test/support/factory.ex`) - Test factories for purchases and webhook events

### Test Suite
- **Purchase tests** (`test/saas_template/purchases_test.exs`) - Unit tests for purchase context
- **Webhook handler tests** (`test/saas_template_web/lemon_squeezy_webhook_handler_test.exs`) - Integration tests for webhook handling
...
1. **Customize purchase processing** - Modify the `SaasTemplate.Purchases` context to fit your business logic
