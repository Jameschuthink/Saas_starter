...
### Files Created
- **`lib/saas_template/ai.ex`**: Main AI context module with example functions
- **`lib/saas_template/ai/llm.ex`**: Core LLM interface module with chain management
...
{:ok, response} = SaasTemplate.AI.LLM.query_text(chain, "What is the capital of France?")
{:ok, parsed} = SaasTemplate.AI.LLM.parse_output(response)
...
json_chain = SaasTemplate.AI.LLM.create_chain("You are a helpful assistant. Respond in JSON format.", json_response: true)
{:ok, response} = SaasTemplate.AI.LLM.query_json(json_chain, "What is the capital of France?")
{:ok, parsed} = SaasTemplate.AI.LLM.parse_output(response)
...
chain = SaasTemplate.AI.LLM.create_chain("You are a helpful assistant.", model: "gpt-3.5-turbo")
```
...
```elixir
# Using the convenience functions from SaasTemplate.AI
{:ok, response} = SaasTemplate.AI.example_query()
...
{:ok, parsed_data} = SaasTemplate.AI.example_json_query()
