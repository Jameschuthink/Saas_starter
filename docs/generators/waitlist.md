...
1. **`lib/saas_template/waitlist/entry.ex`** - Waitlist entry schema with validations
   - Stores email, name, company, role, use_case, and subscription timestamp
   - Validates email format and uniqueness
   - Automatically sets subscribed_at timestamp

2. **`lib/saas_template_web/components/waitlist_components.ex`** - UI components
   - Three different waitlist form styles
   - Helper functions for feature flag checking
   - CSRF protection integration

3. **`lib/saas_template_web/controllers/waitlist_controller.ex`** - Form submission handler
...
1. **`lib/saas_template_web/router.ex`** - Adds waitlist route
   - `POST /waitlist` route for form submissions

2. **`lib/saas_template_web/page_html.ex`** - Imports waitlist components
...
defmodule SaasTemplate.Admin.WaitlistManager do
