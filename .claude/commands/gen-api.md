...
1. **Analyze existing API patterns**:
   - Check `lib/saas_template_web/router.ex` for existing API pipeline
   - Look at `lib/saas_template_web/controllers/` for controller patterns
   - Review `lib/saas_template_web/controllers/error_json.ex` for error handling
   - Check authentication patterns in `lib/saas_template_web/user_auth.ex`

2. **Generate API with proper structure**:
   - Controller module in `lib/saas_template_web/controllers/api/`
   - JSON view module in `lib/saas_template_web/controllers/api/`
   - Test file in `test/saas_template_web/controllers/api/`
   - Route configuration in `router.ex` under `:api` pipeline

3. **Follow template conventions**:
   - Use `SaasTemplateWeb, :controller` for controllers
   - Use `SaasTemplateWeb, :view` for JSON views
...
6. **JSON response patterns**:
   ...
   |> put_view(SaasTemplateWeb.ErrorJSON)
   |> render(:"422")
   ```
