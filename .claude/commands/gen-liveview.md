...
1. **Analyze existing LiveView patterns**:
   - Look at `lib/saas_template_web/live/` for existing LiveView structure
   - Check `lib/saas_template_web/live/user_live/` for authentication patterns
   - Review `lib/saas_template_web/components/core_components.ex` for available components
   - Check `lib/saas_template_web/user_auth.ex` for authentication helpers

2. **Generate LiveView with proper structure**:
   - LiveView module in `lib/saas_template_web/live/[context]_live/`
   - Template file `.html.heex` in the same directory
   - Test file in `test/saas_template_web/live/[context]_live/`
   - Route configuration in `router.ex`

3. **Follow template conventions**:
   - Use `SaasTemplateWeb, :live_view` for the LiveView module
...
5. **Authentication patterns**:
   - Use `on_mount: {SaasTemplateWeb.UserAuth, :mount_current_user}` for optional auth
   - Use `on_mount: {SaasTemplateWeb.UserAuth, :ensure_authenticated}` for required auth
   - Use `on_mount: {SaasTemplateWeb.UserAuth, :redirect_if_user_is_authenticated}` for guest-only
