defmodule SaasTemplateWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: SaasTemplateWeb.Gettext
  ...
  def product_logo(assigns) do
    ...
    <svg
      ...
      fill="none"
      class="size-8"
      alt={Application.get_env(:saas_template, :app_name) <> " logo"}
      aria-label={Application.get_env(:saas_template, :app_name) <> " logo"}
    >
      <path
        fill-rule="evenodd"
        clip-rule="evenodd"
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(SaasTemplateWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(SaasTemplateWeb.Gettext, "errors", msg, opts)
    end
  end
