defmodule Mix.Tasks.SaasTemplate.Gen.Analytics do
  @moduledoc """
  ...
  - Creates the analytics migration
      ...
      $ mix saas_template.gen.analytics
    ...
    config :phoenix_analytics,
      ...
      postgres_conn:
        System.get_env("POSTGRES_CONN") ||
          "dbname=saas_template_dev user=postgres password=postgres host=localhost",
      in_memory: true
    """
    ...
    Igniter.update_file(igniter, "lib/saas_template_web/endpoint.ex", fn source ->
      ...
      if String.contains?(content, "plug PhoenixAnalytics.Plugs.RequestTracker") do
        ...
        updated_content =
          String.replace(
            content,
            ~r/(plug Plug\.Static,[\s\S]*?only: SaasTemplateWeb\.static_paths\(\))/,
  ...
  defp update_router_with_analytics(igniter) do
    igniter
    |> Igniter.update_file("lib/saas_template_web/router.ex", fn source ->
      content = Rewrite.Source.get(source, :content)
    ...
    |> Igniter.update_file("lib/saas_template_web/router.ex", fn source ->
      ...
      if String.contains?(content, "phoenix_analytics_dashboard(\"/analytics\")") do
        ...
        updated_content =
          String.replace(
            content,
            ~r/(get "\/design-system", SaasTemplateWeb\.PageController, :design_system)/,
  ...
  defp create_analytics_migration(igniter) do
    ...
    migration_content = """
    defmodule SaasTemplate.Repo.Migrations.AddPhoenixAnalytics do
      use Ecto.Migration
  ...
  defp print_completion_notice(igniter) do
    ...
    - Analytics dashboard route added at /admin/analytics
    ...
    2. Visit `/admin/analytics` in development to see the dashboard
    ...
    ### Analytics Dashboard:
    - Available at: http://localhost:4000/admin/analytics
