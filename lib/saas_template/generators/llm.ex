defmodule Mix.Tasks.SaasTemplate.Gen.Llm do
  @moduledoc """
  ...
  defp create_ai_context_module(igniter) do
    ai_content = """
    defmodule SaasTemplate.AI do
      alias SaasTemplate.AI.LLM
  ...
  defp create_llm_module(igniter) do
    llm_content = """
    defmodule SaasTemplate.AI.LLM do
      ...
      ```elixir
      chain = SaasTemplate.AI.LLM.create_chain("You are a helpful assistant.")
      SaasTemplate.AI.LLM.query_text(chain, "What is the capital of France?")
      {:ok, "The capital of France is Paris."}

      json_chain = SaasTemplate.AI.LLM.create_chain("You are a helpful assistant.", json_response: true)
      SaasTemplate.AI.LLM.query_json(json_chain, "What is the capital of France?")
      {:ok, %{"capital" => "Paris"}}
      ```
      \"\"\"
  ...
  defp print_completion_notice(igniter) do
    ...
    ### Code Created:
    - SaasTemplate.AI.LLM module for LLM interactions
    - SaasTemplate.AI context module with example functions
    - Support for both text and JSON responses
    - Chain management for conversational AI

    ### Files Created:
    - lib/saas_template/ai.ex - AI context module with examples
    - lib/saas_template/ai/llm.ex - Main LLM interface module
    ...
    2. Usage examples:
       ...
       chain = SaasTemplate.AI.LLM.create_chain("You are a helpful assistant.")
       {:ok, response} = SaasTemplate.AI.LLM.query_text(chain, "Hello!")

       # Create a JSON response chain
       json_chain = SaasTemplate.AI.LLM.create_chain("Respond in JSON format.", json_response: true)
       {:ok, parsed} = SaasTemplate.AI.LLM.query_json(json_chain, "Tell me about Paris")
       ```
