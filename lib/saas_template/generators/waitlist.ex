defmodule Mix.Tasks.SaasTemplate.Gen.Waitlist do
  @moduledoc """
  ...
  defp create_waitlist_schema(igniter, app_name) do
    schema_content = """
    defmodule SaasTemplate.Waitlist.Entry do
      @moduledoc \"\"\"
  ...
  defp create_waitlist_migration(igniter) do
    ...
    migration_content = """
    defmodule SaasTemplate.Repo.Migrations.CreateWaitlistEntries do
      use Ecto.Migration
  ...
  defp create_waitlist_controller(igniter, app_name) do
    controller_content = """
    defmodule SaasTemplateWeb.WaitlistController do
      use SaasTemplateWeb, :controller

      alias SaasTemplate.Waitlist.Entry
  ...
  defp update_web(igniter) do
    Igniter.update_file(igniter, "lib/saas_template_web.ex", fn source ->
      content = Rewrite.Source.get(source, :content)

      if String.contains?(content, "import SaasTemplateWeb.WaitlistComponents") do
        ...
        updated_content =
          String.replace(
            content,
            ~r/(import SaasTemplateWeb\.CoreComponents)/,
            "\\1\n      import SaasTemplateWeb.WaitlistComponents"
          )
