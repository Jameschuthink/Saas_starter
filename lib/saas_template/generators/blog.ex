defmodule Mix.Tasks.SaasTemplate.Gen.Blog do
  @moduledoc """
  ...
  defp update_config(igniter) do
    config_content = """
    config :backpex, :pubsub_server, SaasTemplate.PubSub

    config :backpex,
      translator_function: {SaasTemplateWeb.CoreComponents, :translate_backpex},
      error_translator_function: {SaasTemplateWeb.CoreComponents, :translate_error}
    """
  ...
  defp create_blog_context(igniter) do
    ...
    defmodule SaasTemplate.Blog do
      import Ecto.Query, warn: false
      alias SaasTemplate.Repo
      alias SaasTemplate.Blog.Post
    ...
    igniter
    |> Igniter.create_new_file("lib/saas_template/blog.ex", blog_context_content)
  end

  defp create_blog_post_schema(igniter) do
    post_schema_content = """
    defmodule SaasTemplate.Blog.Post do
    ...
    igniter
    |> Igniter.create_new_file("lib/saas_template/blog/post.ex", post_schema_content)
  end

  defp create_blog_controller(igniter) do
    controller_content = """
    defmodule SaasTemplateWeb.BlogController do
      ...
      def index(conn, _params) do
        ...
        conn
        |> put_meta_tags(%{
          title: "Blog - \#{Application.get_env(:saas_template, :app_name)}",
      ...
      def show(conn, %{"slug" => slug}) do
        case Blog.get_post_by_slug(slug) do
          nil ->
            ...
            |> put_view(SaasTemplateWeb.ErrorHTML)
            |> render(:"404")

          post ->
            conn
            |> put_meta_tags(%{
              title: "\#{post.title} - \#{Application.get_env(:saas_template, :app_name)}",
    ...
    |> Igniter.create_new_file(
      "lib/saas_template_web/controllers/blog_controller.ex",
      controller_content
    )
  end
  ...
  defp create_blog_html(igniter) do
    html_content = """
    defmodule SaasTemplateWeb.BlogHTML do
    ...
    igniter
    |> Igniter.create_new_file(
      "lib/saas_template_web/controllers/blog_html.ex",
      html_content
    )
  end
  ...
  defp create_blog_html_templates(igniter) do
    ...
    <main class="grow">
      <div class="max-w-4xl mx-auto px-4 py-8 mt-24">
        ...
        <article class="prose prose-lg max-w-none">
          {raw(SaasTemplate.Blog.Post.render_content(@post))}
        </article>
    ...
    igniter
    |> Igniter.create_new_file(
      "lib/saas_template_web/controllers/blog_html/index.html.heex",
      index_template
    )
    |> Igniter.create_new_file(
      "lib/saas_template_web/controllers/blog_html/show.html.heex",
      show_template
    )
  end
  ...
  defp create_admin_post_live(igniter) do
    admin_live_content = """
    defmodule SaasTemplateWeb.Live.Admin.PostLive do
        use Backpex.LiveResource,
          layout: {SaasTemplateWeb.Layouts, :blog},
          adapter_config: [
            schema: SaasTemplate.Blog.Post,
            repo: SaasTemplate.Repo,
            update_changeset: &SaasTemplate.Blog.Post.changeset/3,
            create_changeset: &SaasTemplate.Blog.Post.changeset/3
          ]
    ...
    igniter
    |> Igniter.create_new_file(
      "lib/saas_template_web/live/admin/post_live.ex",
      admin_live_content
    )
  end
  ...
  defp update_root_html(igniter) do
    Igniter.update_file(
      igniter,
      "lib/saas_template_web/components/layouts/root.html.heex",
      fn source ->
        content = Rewrite.Source.get(source, :content)
    ...
    Igniter.update_file(igniter, "lib/saas_template_web/router.ex", fn source ->
      ...
      if String.contains?(content, "get \"/blog\"") do
        ...
        updated_content =
          content
          |> String.replace(
            ~r/(import SaasTemplateWeb\.UserAuth)/,
        ...
        if String.contains?(content, "translate_backpex") do
          ...
          translation_functions = """
            ...
            def translate_backpex({msg, bindings}) do
              Gettext.dgettext(SaasTemplateWeb.Gettext, "backpex", msg, bindings)
            end

            def translate_backpex(msg) when is_binary(msg) do
              Gettext.dgettext(SaasTemplateWeb.Gettext, "backpex", msg)
            end
          """
  ...
  defp update_sitemap_controller(igniter) do
    ...
    sitemap_controller_path = "lib/saas_template_web/controllers/sitemap_controller.ex"
      ...
      if String.contains?(content, "add_blog_urls") do
        ...
        blog_function = ~s"""
          defp add_blog_urls(urls, _conn) do
            blog_urls = SaasTemplate.Blog.list_published_posts()
            |> Enum.map(fn post ->
              %{
  ...
  defp print_completion_notice(igniter) do
    ...
    ### Files Created:
    - lib/saas_template/blog.ex - Blog context
    - lib/saas_template/blog/post.ex - Post schema
    - lib/saas_template_web/controllers/blog_controller.ex - Blog controller
    - lib/saas_template_web/controllers/blog_html.ex - Blog HTML module
    - lib/saas_template_web/controllers/blog_html/index.html.heex - Blog index template
    - lib/saas_template_web/controllers/blog_html/show.html.heex - Blog post template
    - lib/saas_template_web/live/admin/post_live.ex - Admin post management
    - lib/saas_template_web/controllers/sitemap_controller.ex - Sitemap generation
