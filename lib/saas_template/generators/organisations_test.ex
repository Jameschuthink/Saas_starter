defmodule Mix.Tasks.SaasTemplate.Gen.OrganisationsTest do
  @moduledoc """
  ...
  - Updated registration tests with organisation handling
      ...
      $ mix saas_template.gen.organisations_test
  ...
  defp create_organisations_context_test(igniter) do
    ...
    defmodule SaasTemplate.OrganisationsTest do
      use SaasTemplate.DataCase

      alias SaasTemplate.Organisations
      alias SaasTemplate.Organisations.{OrganisationMember, OrganisationInvitation}
      alias SaasTemplate.Repo

      import SaasTemplate.Factory
    ...
    Igniter.create_new_file(
      igniter,
      "test/saas_template/organisations_test.exs",
      organisations_test_content
    )
  end
  ...
  defp create_organisations_live_new_test(igniter) do
    new_test_content = """
    defmodule SaasTemplateWeb.OrganisationsLive.NewTest do
      use SaasTemplateWeb.ConnCase
      ...
      describe "form behavior" do
        test "can create organisation with valid data", %{conn: conn} do
          ...
          organisation = SaasTemplate.Organisations.get_by_user_id(user.id)
          assert organisation.name == "New Organisation"
          assert SaasTemplate.Organisations.get_user_role(organisation, user) == "owner"
        end
    ...
    Igniter.create_new_file(
      igniter,
      "test/saas_template_web/live/organisations_live/new_test.exs",
      new_test_content
    )
  end
  ...
  defp create_organisations_live_manage_test(igniter) do
    manage_test_content = """
    defmodule SaasTemplateWeb.OrganisationsLive.ManageTest do
      use SaasTemplateWeb.ConnCase
    ...
    Igniter.create_new_file(
      igniter,
      "test/saas_template_web/live/organisations_live/manage_test.exs",
      manage_test_content
    )
  end
  ...
  defp create_organisations_live_invitation_test(igniter) do
    ...
    defmodule SaasTemplateWeb.OrganisationsLive.InvitationTest do
      use SaasTemplateWeb.ConnCase
      ...
      describe "acceptance flow" do
        test "can accept invitation for new user", %{conn: conn} do
          ...
          user = SaasTemplate.Accounts.get_user_by_email("newuser@test.com")
          assert user

          # Verify user was added to organisation with correct role
          role = SaasTemplate.Organisations.get_user_role(organisation, user)
          assert role == "admin"

          # Verify invitation was marked as accepted
          updated_invitation =
            SaasTemplate.Repo.get(SaasTemplate.Organisations.OrganisationInvitation, invitation.id)
        ...
        test "can accept invitation for existing user", %{conn: conn} do
          ...
          # Verify user was added to organisation
          role = SaasTemplate.Organisations.get_user_role(organisation, existing_user)
          assert role == "member"
    ...
    Igniter.create_new_file(
      igniter,
      "test/saas_template_web/live/organisations_live/invitation_test.exs",
      invitation_test_content
    )
  end

  defp create_user_auth_test(igniter) do
    Igniter.update_file(igniter, "test/saas_template_web/user_auth_test.exs", fn source ->
      ...
      if String.contains?(content, "assign_org_to_scope/2") do
        ...
        content_with_factory =
          String.replace(
            content,
            "  import SaasTemplate.AccountsFixtures",
            "  import SaasTemplate.AccountsFixtures\n  import SaasTemplate.Factory"
          )
          ...
          describe "on_mount :require_organisation" do
            ...
            test "halts and redirects when user has no organisation" do
              ...
              socket = %Phoenix.LiveView.Socket{
                endpoint: SaasTemplateWeb.Endpoint,
  ...
  defp create_scope_test(igniter) do
    scope_test_content = """
      defmodule SaasTemplate.Accounts.ScopeTest do
        use SaasTemplate.DataCase
    ...
    Igniter.create_new_file(
      igniter,
      "test/saas_template/accounts/scope_test.exs",
      scope_test_content
    )
  end
  ...
  defp update_factory(igniter) do
    Igniter.update_file(igniter, "test/support/factory.ex", fn source ->
      ...
      if String.contains?(content, "organisation_factory") do
        ...
        alias SaasTemplate.Organisations.{Organisation, OrganisationMember, OrganisationInvitation}
        ...
        updated_content =
          String.replace(
            content,
            "  use ExMachina.Ecto, repo: SaasTemplate.Repo",
            "  use ExMachina.Ecto, repo: SaasTemplate.Repo" <> factory_additions
          )
  ...
  defp print_completion_notice(igniter) do
    ...
    - test/saas_template/organisations/organisation_test.exs
    - test/saas_template/organisations/organisation_member_test.exs
    - test/saas_template/organisations/organisation_invitation_test.exs
    - test/saas_template/organisations_test.exs
    - test/saas_template_web/live/organisations_live/new_test.exs
    - test/saas_template_web/live/organisations_live/manage_test.exs
    - test/saas_template_web/live/organisations_live/invitation_test.exs
    - test/saas_template_web/user_auth_test.exs (updated)
    - test/saas_template/accounts/scope_test.exs (updated)
