defmodule Mix.Tasks.SaasTemplate.Gen.Lemonsqueezy do
  @moduledoc """
  ...
  - Updates .env.example with LemonSqueezy environment variables
      ...
      $ mix saas_template.gen.lemonsqueezy
  ...
  defp create_purchase_schema(igniter) do
    purchase_content = """
    defmodule SaasTemplate.Purchases.Purchase do
      ...
      schema "purchases" do
        ...
        belongs_to :user, SaasTemplate.Accounts.User
    ...
    Igniter.create_new_file(
      igniter,
      "lib/saas_template/purchases/purchase.ex",
      purchase_content
    )
  end
  ...
  defp create_purchases_context(igniter) do
    purchases_content = """
    defmodule SaasTemplate.Purchases do
      import Ecto.Query, warn: false
      alias SaasTemplate.Repo
      alias SaasTemplate.Purchases.Purchase
      require Logger
    ...
    Igniter.create_new_file(
      igniter,
      "lib/saas_template/purchases.ex",
      purchases_content
    )
  end
  ...
  defp create_lemonsqueezy_webhook_handler(igniter) do
    handler_content = """
    defmodule SaasTemplateWeb.LemonSqueezyWebhookHandler do
      alias SaasTemplate.Purchases
      require Logger
    ...
    Igniter.create_new_file(
      igniter,
      "lib/saas_template_web/controllers/lemon_squeezy_webhooks_controller.ex",
      handler_content
    )
  end

  defp update_endpoint(igniter) do
    Igniter.update_file(igniter, "lib/saas_template_web/endpoint.ex", fn source ->
      content = Rewrite.Source.get(source, :content)
  ...
  defp create_migration(igniter) do
    ...
    migration_content = """
    defmodule SaasTemplate.Repo.Migrations.AddPurchases do
      use Ecto.Migration
  ...
  defp create_purchases_test(igniter) do
    test_content = """
    defmodule SaasTemplate.PurchasesTest do
    ...
    Igniter.create_new_file(
      igniter,
      "test/saas_template/purchases_test.exs",
      test_content
    )
  end
  ...
  defp create_webhook_handler_test(igniter) do
    test_content = """
    defmodule SaasTemplateWeb.LemonSqueezyWebhookHandlerTest do
      use SaasTemplate.DataCase
      alias SaasTemplateWeb.LemonSqueezyWebhookHandler
      alias SaasTemplate.Purchases
      import SaasTemplate.Factory
    ...
    Igniter.create_new_file(
      igniter,
      "test/saas_template_web/lemon_squeezy_webhook_handler_test.exs",
      test_content
    )
  end
  ...
  defp update_factory(igniter) do
    Igniter.update_file(igniter, "test/support/factory.ex", fn source ->
      ...
      if String.contains?(content, "def purchase_factory") do
        ...
        replacement = "use ExMachina.Ecto, repo: SaasTemplate.Repo\n" <> factory_definitions
  ...
  defp print_completion_notice(igniter) do
    ...
    ### Code Created:
    - SaasTemplate.Purchases context module for purchase management
    - SaasTemplate.Purchases.Purchase schema with validations
    - LemonSqueezyWebhookHandler for handling LemonSqueezy webhook events
    - Webhook plug at /webhook/lemonsqueezy

    ### Files Created:
    - lib/saas_template/purchases.ex - Purchase context module
    - lib/saas_template/purchases/purchase.ex - Purchase schema
    - lib/saas_template_web/controllers/lemon_squeezy_webhooks_controller.ex - Webhook handler
    ...
    - lib/saas_template_web/endpoint.ex with webhook plug
    ...
    ```elixir
    # List all purchases
    SaasTemplate.Purchases.list_purchases()

    # Get purchase by LemonSqueezy order ID
    SaasTemplate.Purchases.get_purchase_by_lemonsqueezy_order_id(123456)

    # Get purchase by order identifier
    SaasTemplate.Purchases.get_purchase_by_order_identifier("uuid-123-456-789")
