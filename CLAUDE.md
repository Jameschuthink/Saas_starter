...
mix saas_template.setup     # Modular application setup
...
- **SaasTemplate.Application**: OTP supervision tree with telemetry, database, PubSub, and web endpoint
- **SaasTemplateWeb**: Web layer containing controllers, components, and LiveView functionality
...
- **Development DB**: `saas_template_dev` with postgres/postgres credentials
...
- **Check CoreComponents First**: Always examine `lib/saas_template_web/components/core_components.ex` before creating new reusable UI components
...
- The `mix saas_template.setup` script is designed to be run once on a fresh template
